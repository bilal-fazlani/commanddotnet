...
public class DefinedMiddlewareStepsTests(ITestOutputHelper output)
{
    ...
    public void MiddlewareSteps_Should_HaveUniqueOrder()
    {
        ...
        if (duplicatedSteps.Count == 0) return;
        output.WriteLine("Duplicated steps");
        PrintSteps(duplicatedSteps);

        output.WriteLine("");
        output.WriteLine("All steps");
        PrintSteps(definedSteps);

        duplicatedSteps.Should().BeEmpty();
    }

    private void PrintSteps(ICollection<Step> steps)
    {
    ...
    private record Step(string name, MiddlewareStep step)
    {
        public override string ToString() => $"Step: {name} > {step.Stage}:{step.OrderWithinStage}";
    }
}