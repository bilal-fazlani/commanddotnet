...
The middleware pipeline is a type of [chain of responsibility pattern](https://en.wikipedia.org/wiki/Chain-of-responsibility_pattern).  
...
As the middleware executes, it enhances the _CommandContext_.  The _CommandContext_ contains the context relevant to the current command. For example... the tokenizer middleware populates the _CommandContext.Tokens_ property.  The parser middleware populates the _CommandContext.ParseResults_ property.  The _CommandContext_ is part of the delegate signature and is therefore available for all middleware.

### Why we use the middleware pattern
* Ease of adding new features without touching existing code. Observing [Single Responsibility](https://en.wikipedia.org/wiki/Single_responsibility_principle) &  [Open-Closed](https://en.wikipedia.org/wiki/Open%E2%80%93closed_principle) principles.
