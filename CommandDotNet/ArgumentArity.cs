...
public class ArgumentArity : IArgumentArity
{
    ...
    public override string ToString() => $"{nameof(ArgumentArity)}:{Minimum}..{Maximum}";

    // ReSharper disable once RedundantCast
    public static bool operator ==(ArgumentArity x, ArgumentArity y) => (object)x == y;

    public static bool operator !=(ArgumentArity x, ArgumentArity y) => !(x == y);

    protected bool Equals(ArgumentArity other) => Minimum == other.Minimum && Maximum == other.Maximum;

    public override bool Equals(object? obj) =>
        obj is not null
        && (ReferenceEquals(this, obj)
            || obj.GetType() == GetType()
            && Equals((ArgumentArity) obj));
