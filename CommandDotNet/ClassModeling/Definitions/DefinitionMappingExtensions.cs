...
internal static class DefinitionMappingExtensions
{
    ...
    private static IArgument BuildArgument(IArgumentDef argumentDef, TypeInfo typeInfo, bool isInterceptorOption)
    {
        ...
        if (argumentDef.CommandNodeType == CommandNodeType.Operand)
        {
            ...
            SetDescription(operand, argumentDef, operandAttr);
            return operand;
        }
        ...
        INameAndDescription? nameAndDescription)
    {
        ...
        var description = nameAndDescription?.Description;
        var descriptionLines = nameAndDescription?.DescriptionLines;

        // Count how many description sources are set
        var descriptionCount = 0;
        if (description is not null) descriptionCount++;
        if (descriptionLines is not null) descriptionCount++;
        if (descriptionMethodAttr is not null) descriptionCount++;

        if (descriptionCount > 1)
        {
            var setProperties = new List<string>();
            if (description is not null) setProperties.Add(nameof(INameAndDescription.Description));
            if (descriptionLines is not null) setProperties.Add(nameof(INameAndDescription.DescriptionLines));
        ...
        else
        {
            ...
            var staticDescription = description ?? descriptionLines?.ToCsv(Environment.NewLine);
            if (argument is Option option)
            {
                option.Description = staticDescription;
            }
            else if (argument is Operand operand)
            {
                operand.Description = staticDescription;
            }
        }
    }
