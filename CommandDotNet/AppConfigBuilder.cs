...
public class AppConfigBuilder
{
    ...
    {
        AppSettings = appSettings.ThrowIfNull();
    ...
    private class SingleRegistrationGuard<T>(string type, Func<T, string?> getName)
        where T : notnull
    {
        private readonly string _type = type.ThrowIfNull();
        private readonly Func<T, string?> _getName = getName.ThrowIfNull();
        ...
        internal void Register(T key)
        {
            if (_registrations.TryGetValue(key, out var info))
            {
                ...
                if (AppRunnerConfigExtensions.InUseDefaultMiddleware || info.InUseDefaultMiddleware)
                {
        ...
        {
            public readonly string? ExcludeParamName = AppRunnerConfigExtensions.ExcludeParamName;
